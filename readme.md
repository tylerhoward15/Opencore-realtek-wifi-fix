# Background
This is a tool that was released a long time ago and is no longer actively maintained to my knowledge. This tool defaults to installing the kexts and attaching to a clover installation, but it is fairly easy to use it on an OpenCore setup. At the time of writing, these kexts and installer are working on the latest version of MacOs: Catalina 10.15.4.

# How to install
- Download `Wireless USB Adapter Clover-V14.pkg and go through the installation wizard

If using Clover, you are good to go, the wizard will take care of everything. If you are using OpenCore, continue below.

- The wifi utility will now be installed in macos available in the status bar, however the kexts will be installed for clover configurations and won't be loaded for opencore without some changes

- Mount your EFI partition that boots your hackintosh, and move the kexts `RtWlanU.kext` and `RtWlanU1827.kext` from `EFI/CLOVER/kexts/Other` into `EFI/OC/Kexts`

- Now you need to inject your new kexts. You do this by editing your `config.plist` file to include the two new kexts.

- In the section under Kernels/Add, add this section to the array of kexts listed:
...
- When you boot back into macos, you should have working wifi!
